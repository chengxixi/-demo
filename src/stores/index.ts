...
export const useFeedbackStore = defineStore('feedback', () => {
    ...
    const mergeGroups = computed<MergeGroup[]>(() => {
        ...
        items.value.forEach((item) => {
            ...
            if (['P0', 'P1'].includes(item.exception)) {
                groups.push({
                    mergeGroup: '',
                    lead: { data: item, confidence: 0.78 },
                    members: [{ data: item, confidence: 0.78 }],
                    expanded: false,
                });
                return;
            }
            ...
            if (members.length > 1) {
                groups.push({
                    ...
                    lead: { data: item, confidence: 0.78 },
                    members: members.map((m) => ({ data: m, confidence: 0.78 })),
                    expanded: false,
                });
            } else {
                groups.push({
    ...
    const filteredGroups = computed(() => {
        ...
        if (activeTab.value === 'pending') {
            filtered = filtered.filter(
                (g) =>
                    g.lead.data.processState === '待处理' ||
                    g.lead.data.processState === '待人工复核',
            );
        ...
        if (searchQuery.value) {
            ...
            filtered = filtered.filter((g) =>
                g.members.some((m) => Object.values(m.data).join(' ').toLowerCase().includes(q)),
            );
        }
        ...
        if (f.source)
            filtered = filtered.filter((g) =>
                g.members.some((m) => getDataSource(m.data) === f.source),
            );
        if (f.mode)
            filtered = filtered.filter((g) => g.members.some((m) => m.data.createMode === f.mode));
        if (f.brand)
            filtered = filtered.filter((g) => g.members.some((m) => m.data.brand === f.brand));
        if (f.site)
            filtered = filtered.filter((g) => g.members.some((m) => m.data.site === f.site));
        if (f.productType)
            filtered = filtered.filter((g) =>
                g.members.some((m) => m.data.productType === f.productType),
            );
        if (f.model)
            filtered = filtered.filter((g) => g.members.some((m) => m.data.model === f.model));
        if (f.internal)
            filtered = filtered.filter((g) =>
                g.members.some((m) => m.data.internal === f.internal),
            );
        if (f.feedbackUser)
            filtered = filtered.filter((g) =>
                g.members.some((m) => m.data.feedbackUser === f.feedbackUser),
            );
        if (f.processState)
            filtered = filtered.filter((g) =>
                g.members.some((m) => m.data.processState === f.processState),
            );
        if (f.exception)
            filtered = filtered.filter((g) =>
                g.members.some((m) => m.data.exception === f.exception),
            );
        if (f.returned)
            filtered = filtered.filter((g) =>
                g.members.some((m) => m.data.returned === f.returned),
            );
        if (f.processRoute)
            filtered = filtered.filter((g) =>
                g.members.some((m) => m.data.processRoute === f.processRoute),
            );
    ...
    function getDataSource(data: FeedbackItem): string {
        if (data.source === '退货原因')
            return data.site === 'Amazon.com (US)' ? '海外电商-退货反馈' : '国内电商-退货反馈';
        if (data.source === '商品评论')
            return data.site === 'Amazon.com (US)' ? '海外电商-商品评论' : '国内电商-商品评论';
        return data.source;
    }
